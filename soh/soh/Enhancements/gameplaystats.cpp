...
void DrawGameplayStatsHeader() {
    ...
    if (gPlayState != NULL &&
        CVarGetInteger(CVAR_ENHANCEMENT("GameplayStats.ShowDebugInfo"), 0)) { // && display debug info
...
void DrawGameplayStatsBreakdownTab() {
    for (int i = 0; i < gSaveContext.ship.stats.tsIdx; i++) {
        ...
        std::string name;
        if (CVarGetInteger(CVAR_ENHANCEMENT("GameplayStats.RoomBreakdown"), 0) &&
    ...
    std::string toPass;
    if (CVarGetInteger(CVAR_ENHANCEMENT("GameplayStats.RoomBreakdown"), 0) &&
        gSaveContext.ship.stats.sceneNum != SCENE_GROTTOS) {
...
void SetupDisplayNames() {
    ...
    strcpy(itemTimestampDisplayName[ITEM_LENS],             "Lens of Struth:      ");
    ...
    // CUSTOM
    strcpy(itemTimestampDisplayName[ITEM_GLIDER],      "Glider:           ");
    // Other events
